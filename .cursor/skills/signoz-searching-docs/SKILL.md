...
description: >
  ...
  inside SigNoz, pick the matching `signoz-*` action skill (see
  references/project-context.md). For this repo's self-hosted setup,
  Foundry layout, or v0.133 API quirks, read references/project-context.md
  after fetching official docs.
---
...
For questions about **this repository** (Agent K hackathon, Foundry deploy, local MCP, v0.133 API), read [project-context.md](./references/project-context.md) and reconcile with fetched docs — official docs may lag behind the version pinned here.
...
This project's MCP server is configured in `.cursor/mcp.json` at `http://localhost:8000/mcp` (docker-compose `mcp-server` service). Start with `make up` and ensure bootstrap has written `SIGNOZ_API_KEY` to `.env`.
